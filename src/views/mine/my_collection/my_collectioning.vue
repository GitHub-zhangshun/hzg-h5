            ...
            <header>
                ...
                <ul class="contain">
                    ...
                    <li v-for="(item, index) in list" :key="index" v-else class="yesList">
                        ...
                        <section>
                            <p>待回本金：<span>{{item.principal}}</span></p>
                            <p>待回收益：<span>{{item.earnings}}</span> <i v-if="item.flag" style="color:#ff4e30" @click="diagol(item.damages)" class="icon iconfont icon-gantanhao"></i> <span class="yfj">{{item.increaseInterest==0?'':`+${item.increaseInterest}`}}</span></p>
    ...
    export default {
        ...
        created(){
            indexApi.userRepayStatistics().then(data=>{
                this.interest = publicFun.toMoney(data.result.waitInterest||0,2)
                this.principal = publicFun.toMoney(data.result.waitPrincipal||0,2)
            })
        },
        methods:{
            ...
            upCallback (page, mescroll) {
                ...
                indexApi.userRepayList(formdata).then(data =>{
                    ...
                    for(var i = 0; i<data.result.length;i++){
                        let flag = false
                        if(data.result[i].damages!=0){
                            flag = true
                        }
                        this.list.push({
                            flag:flag,
                            bidName:data.result[i].bidName,
                            damages:publicFun.toMoney(data.result[i].damages||0,2),
                            earnings:publicFun.numAdd(data.result[i].interest,data.result[i].damages),
